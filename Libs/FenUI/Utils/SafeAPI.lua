...
function Utils:GetSpellCooldownSafe(spellID)
    if not spellID or not _G.C_Spell or not _G.C_Spell.GetSpellCooldown then return nil end
    InvalidateCache()
...
function Utils:GetSpecializationSafe()
    if self.IS_MIDNIGHT and _G.C_SpecializationInfo and _G.C_SpecializationInfo.GetSpecialization then
        ...
        if ok then return _G.tonumber(spec) or spec end
    end
...
function Utils:GetSpellChargesSafe(spellID)
    if not spellID or not _G.C_Spell or not _G.C_Spell.GetSpellCharges then return nil end
    InvalidateCache()
...
function Utils:GetSpellTextureSafe(spellID)
    if not spellID or not _G.C_Spell or not _G.C_Spell.GetSpellTexture then return nil end
...
function Utils:GetItemSpellSafe(itemInfo)
    if not itemInfo or not _G.C_Item or not _G.C_Item.GetItemSpell then return nil end
