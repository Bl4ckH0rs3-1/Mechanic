...
function InspectModule:AddDetailGeometry(frame, yOffset)
	...
	local function safeCall(obj, method, ...)
		if obj[method] then
			...
			if ok then return val, val2 end
		end
		return nil
	end
...
function InspectModule:AddDetailFenUI(frame, yOffset)
	...
	if frame.borderApplied ~= nil then
		table.insert(details, string.format("Border: %s", frame.borderApplied and "|cff00ff00Applied|r" or "|cff888888None|r"))
	end
	...
	if frame.config and type(frame.config) == "table" then
		...
		if frame.config.background then
			...
			elseif type(bg) == "table" then
				if bg.color then
					table.insert(details, string.format("Config BG: |cffffffff%s|r (alpha %.2f)", tostring(bg.color), bg.alpha or 1))
		...
		if frame.config.padding then
			...
			if type(p) == "table" then
				table.insert(details, string.format("Padding: L:%s R:%s T:%s B:%s", tostring(p.left or 0), tostring(p.right or 0), tostring(p.top or 0), tostring(p.bottom or 0)))
			else
...
function InspectModule:GetOrCreateDetailSection(name, yOffset)
	local section = Mechanic.Utils:GetOrCreateWidget(self.detailsContent, "section_" .. name, function(p)
		...
		if not s.content:GetFont() then s.content:SetFontObject("ChatFontNormal") end
