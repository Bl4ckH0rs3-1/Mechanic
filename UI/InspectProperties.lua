...
function Properties.inputs:Dropdown(parent, label, options, selected, onChange)
	...
	-- Use a simple button that opens a menu for now, as UIDropDownMenu is a bit heavy
	...
	btn:SetScript("OnClick", function()
		...
		for _, opt in ipairs(options) do
			table.insert(menu, {
				...
				checked = (opt == btn.text:GetText())
			})
		end
		-- EasyMenu is deprecated in 11.0, using Menu interface if available or just a simple popup
...
function Properties.inputs:Color(parent, label, r, g, b, a, onChange)
	...
	bg:SetAllPoints()
	bg:SetColorTexture(1, 1, 1, 1) -- Checkerboard background would be better but white is fine
	...
	swatch:SetScript("OnClick", function()
		...
		if ColorPickerFrame.SetupColorPickerAndShow then
			...
			-- Fallback for older versions if needed, but we target 12.0
...
function Properties:InitializeDefaultSections()
	-- ... existing geometry, visibility, layering, scale sections ...
	-- (Note: I'll include the full function for safety with search_replace)
	...
	self:RegisterSection("visibility", {
		...
		createUI = function(parent, frame)
			...
			local shownInput = self.inputs:Checkbox(parent, L["Shown"], frame:IsShown(), function(val)
			...
			local alphaInput = self.inputs:Slider(parent, L["Alpha"], frame:GetAlpha(), 0, 1, 0.05, function(val)
				frame:SetAlpha(val)
		...
		getExportData = function(frame)
			...
			if self.pendingChanges.shown then
				table.insert(data, { property = "Shown", before = tostring(self.pendingChanges.shown.before), after = tostring(self.pendingChanges.shown.after) })
			end
			if self.pendingChanges.alpha then
				table.insert(data, { property = "Alpha", before = string.format("%.2f", self.pendingChanges.alpha.before), after = string.format("%.2f", self.pendingChanges.alpha.after) })
			end
	...
	self:RegisterSection("layering", {
		...
		createUI = function(parent, frame)
			...
			local levelInput = self.inputs:Number(parent, L["Level"], frame:GetFrameLevel(), function(val)
			...
			local strataInput = self.inputs:Dropdown(parent, L["Strata"], strataOptions, frame:GetFrameStrata(), function(val)
	...
	self:RegisterSection("scale", {
		...
		createUI = function(parent, frame)
			...
			local scaleInput = self.inputs:Slider(parent, L["Scale"], frame:GetScale(), 0.1, 5, 0.1, function(val)
				frame:SetScale(val)
		...
		getExportData = function(frame)
			if self.pendingChanges.scale then
				return {{ property = "Scale", before = string.format("%.2f", self.pendingChanges.scale.before), after = string.format("%.2f", self.pendingChanges.scale.after) }}
			end
		end,
		getExportLua = function(frame)
			if self.pendingChanges.scale then
	...
	self:RegisterSection("texture", {
		...
		createUI = function(parent, frame)
			...
			local colorInput = self.inputs:Color(parent, L["Vertex Color"], r, g, b, a, function(nr, ng, nb, na)
	...
	self:RegisterSection("fontstring", {
		...
		createUI = function(parent, frame)
			...
			local textInput = self.inputs:Text(parent, L["Text"], frame:GetText(), function(val)
				frame:SetText(val)
			...
			local colorInput = self.inputs:Color(parent, L["Text Color"] or "Text Color", r, g, b, a, function(nr, ng, nb, na)
		...
		getExportData = function(frame)
			...
			if self.pendingChanges.text then
				table.insert(data, { property = "Text", before = self.pendingChanges.text.before, after = self.pendingChanges.text.after })
			end
...
function Properties:ExportChanges()
	if not next(self.pendingChanges) then
		Mechanic:Print(L["No changes to export."])
		return
	end
	...
	local title = string.format("%s - %s", L["Export"], path)
